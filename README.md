...
```bash
python main.py spot us-east-1 us-west-2    # Spot analysis
python main.py blocks us-west-2            # Capacity blocks  
python main.py ondemand                    # On-demand (default regions)
python main.py all                         # Full analysis
```
...
```
p-series-aws/
├── main.py                 # Entry point - CLI and menu launcher
├── p_series_menu.py        # Interactive terminal menu (Unix only)
├── requirements.txt        # Python dependencies
│
├── core/                   # Business logic and data
│   ├── advisor.py          # GPUAdvisor - analysis orchestration
│   ├── aws_client.py       # AWS API wrapper with caching
│   └── models.py           # Data classes (SpotResult, etc.)
│
├── cli/                    # Command line interface
│   └── commands.py         # CLI command implementations
│
├── formatters/             # Output formatting
│   └── table_formatter.py  # Table display and progress indicator
│
└── tests/                  # Test suite
...
### Why 24-Hour Capacity Blocks?

The tool queries capacity blocks with a fixed 24-hour duration. This is intentional:

- **Target use case**: Ad-hoc training jobs that need immediate, short-term access
- **Cost efficiency**: 24 hours is the minimum block duration AWS offers, keeping costs low for experimentation
- **Simplicity**: A single duration makes comparison across regions straightforward

If you need longer blocks, modify `CapacityDurationHours` in `aws_client.py:get_capacity_block_offerings()`.

### Why Static On-Demand Availability Matrix?

The `ON_DEMAND_AVAILABILITY` dict in `aws_client.py` is hardcoded rather than queried dynamically:

- **P5+ instances are spot/CB only**: AWS does not offer P5, P5e, P5en, or P6 instances on-demand. This is an AWS policy, not a capacity issue.
- **Avoids unnecessary API calls**: Querying on-demand availability for instances that will never be available wastes time and API quota.
- **Explicit documentation**: The matrix serves as documentation of what's actually purchasable on-demand.

The matrix was last verified January 2026. If AWS changes on-demand availability for P-series, update the `ON_DEMAND_AVAILABILITY` dict.

### Silent API Error Handling

AWS API calls use `except Exception: pass` patterns intentionally:

- **Graceful degradation**: A failed API call for one region shouldn't crash the entire analysis
- **Partial results are useful**: Getting data for 6 of 7 regions is better than getting nothing
- **User experience**: The tool shows "No availability" rather than stack traces

For debugging, run with AWS SDK logging enabled: `export AWS_SDK_LOG_LEVEL=debug`
